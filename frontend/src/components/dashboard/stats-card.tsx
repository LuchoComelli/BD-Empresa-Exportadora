      ...
      <CardContent className="p-4 md:p-6">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <p className="text-xs md:text-sm font-medium text-ministerio-gray">{title}</p>
            <p className="text-2xl md:text-3xl font-bold text-ministerio-navy mt-1 md:mt-2">{value}</p>
            {trend && (
              <p
                className={`text-xs md:text-sm mt-1 md:mt-2 ${
                  trend.isPositive ? 'text-ministerio-yellow' : 'text-red-600'
                }`}
              >
